...
CoinEntity::CoinEntity(EntityManager* l_EntityMgr) :
...
void CoinEntity::Move(const Direction& l_dir) {
...
void CoinEntity::GetHurt(const int& l_Damage) {
	if (GetState() == EntityState::Dying) {
		return;
	}
...
void CoinEntity::Load(const std::string& l_Path) {
	std::ifstream file;
	std::string line;
...
void CoinEntity::Update(float l_deltaTime) {
...
void CoinEntity::Draw(sf::RenderWindow* l_Window) {
